...
async function req<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!r.ok) {
    ...
    const raw = err.error ?? err.detail ?? err;
    // Serializer errors come through as objects like {"width_inches":["..."]} —
    // collapse to a readable string instead of "[object Object]".
    const msg = typeof raw === 'string'
      ? raw
      : (() => {
          try { return JSON.stringify(raw); } catch { return 'Request failed'; }
        })();
