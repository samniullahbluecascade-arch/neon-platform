...
_ADDITIONAL_PATTERN = "Additional Instructions: [ ]"
_UV_PATTERN         = "UV_PRINTING INSTRUCTIONS: [ ]"
# Legacy fallback (old prompts only had a single "[ ]")
_LEGACY_PLACEHOLDER = "[ ]"


def _fill_placeholders(prompt: str, additional: str, uv: str) -> str:
    """
    Substitute the two named placeholders in a prompt template:

      "Additional Instructions: [ ]"     ← additional (from textarea)
      "UV_PRINTING INSTRUCTIONS: [ ]"    ← uv         (from UV checkbox)

    Empty values become "None" so the LLM gets a concrete read.
    Falls back to a single legacy "[ ]" replacement if neither named
    pattern is present (old prompts).
    """
    add_text = additional.strip() or "None"
    uv_text  = uv.strip() or "None"

    if _ADDITIONAL_PATTERN in prompt or _UV_PATTERN in prompt:
        return (prompt
                .replace(_ADDITIONAL_PATTERN, f"Additional Instructions: {add_text}")
                .replace(_UV_PATTERN,         f"UV_PRINTING INSTRUCTIONS: {uv_text}"))

    # Legacy prompt (single "[ ]") — combine both into one block
    combined = add_text if uv_text == "None" else f"{add_text}\n\n{uv_text}"
    if _LEGACY_PLACEHOLDER in prompt:
        return prompt.replace(_LEGACY_PLACEHOLDER, combined)
...
def generate_bw(
    ...
    prompt_text = _fill_placeholders(_MOCKUP_TO_BW_PROMPT, additional, uv)
