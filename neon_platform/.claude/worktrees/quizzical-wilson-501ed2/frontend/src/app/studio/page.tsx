...
export default function StudioPage() {
  ...
  const runStep1 = async () => {
    ...
    // Re-running Phase 1 invalidates everything downstream
  ...
  const runStep2 = async () => {
    ...
    // Re-running Phase 2 invalidates the LOC measurement
    setMeasurement(null);
    ...
    try {
      ...
      setC3State('');           // back to idle so user sees the MEASURE LOC cue
      setStatusMsg('B&W ready. Click MEASURE LOC to remeasure.');
    } catch (e: unknown) {
      setC3State('err');
  ...
  const runStep3 = async () => {
    ...
    // Clear stale measurement + flip Card 3 to active
    setMeasurement(null);
    setC3State('active');
  ...
  const runFullPipeline = async () => {
    ...
    const w = widthNum();
    if (!w) { setError('Enter a valid sign width (inches).'); return; }
    // Hard reset — every full pipeline run is fresh
      ...
      <div style={{ maxWidth: '1380px', margin: '0 auto', padding: '2rem' }}>
        ...
        <div style={{
          ...
          <Card num={1} title={mode === 'full' ? 'Design Input' : 'Mockup Input'} sub={mode === 'full' ? 'Upload logo + optional background' : 'Upload colored mockup'} state={c1State}>
            ...
            />
            {/* UV checkbox — visible on both pipelines */}
            <label style={{
          ...
          {mode === 'full' && (
            <Card num={2} title="Colored Neon Mockup" sub="Gemini renders photorealistic LED neon sign" state={c2State}>
              ...
              <Label optional>Additional Instructions (B&amp;W phase)</Label>
              <textarea
...
function Results({ m }: { m: MeasurementResult }) {
  ...
  return (
    <div className="card" style={{ padding: '1.5rem', marginTop: '1.5rem' }}>
      ...
      {/* Breakeven price (single value, no breakdown) */}
      {(() => {
        ...
        return (
          <div style={{
            ...
            <div>
              <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.25rem' }}>Tube Length</div>
            ...
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.25rem' }}>Breakeven Price</div>
              <div style={{ fontFamily: 'Space Mono, monospace', fontSize: '2.1rem', fontWeight: 700, color: '#00ff9d', textShadow: '0 0 18px rgba(0,255,157,0.45)' }}>
                ${total.toFixed(2)}
              </div>
            </div>
          </div>
