  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '2rem' }}>
              ...
              {job.estimated_price != null && (
                <div style={{
                  ...
                  <div style={{
                    ...
                    ${(job.total_price ?? job.estimated_price).toFixed(2)}
                  </div>
                </div>
              )}
            </div>
        ...
        {/* Feedback collection (done jobs only) */}
        {job.status === 'done' && <JobFeedback jobId={job.id} />}

        {/* Action row */}
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
          <Link href="/dashboard"><button className="btn-ghost">← Back to Dashboard</button></Link>
          {job.status === 'done' && <DownloadReceipt job={job} />}
        </div>
      </div>
    </div>
  );
