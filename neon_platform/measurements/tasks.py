...
def run_measurement(self, job_id: str) -> dict:
    ...
    try:
        ...
        job.estimated_price = round(max(25.0, result.measured_m * 10), 2)
        # Shipping cost — never block on API failure
        try:
            from measurements.shipping import get_shipping_cost
            shipping = get_shipping_cost(float(job.width_inches), img_bytes)
        except Exception as exc:
            logger.warning("Quick Measure shipping calc failed: %s", exc)
            shipping = None
        job.shipping_cost = shipping
        job.total_price   = round(job.estimated_price + (shipping or 0.0), 2)
